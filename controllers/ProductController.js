...
export const saveProduct = (req, res)=>{
    ...
    if(fileSize > 5000000) return res.status(422).json({msg: "Image Should be less than 5 MB"});
...
export const updateProduct = async(req, res)=>{
    ...
    if(req.files === null){
        ...
        if(!allowedTyped.includes(ext.toLowerCase())) return res.status(404).json({msg:"Invalid Images"});
        if(fileSize > 5000000) return res.status(422).json({msg: "Image Should be less than 5 MB"});
